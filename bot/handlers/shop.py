        ...
        async with async_session() as session:
            try:
                ...
                try:
                    ...
                    pass
                await message.answer(f"⚠️ خطا در پر کردن مغازه: {escape(type(e1).__name__)}: {escape(str(e1))}")
            try:
...
async def cmd_teahouse(message: Message):
    ...
    for item in chunk:
        ...
        text += f"• <b>{item_name}</b>" + chr(10) + f"  {desc}" + chr(10) + f"  قیمت: <b>{escape(str(item.price or 0))}</b> سکه" + chr(10) + chr(10)
...
async def _render_building(callback: CallbackQuery, owner_id: int, building_id: int, page: int = 0):
    ...
    bname = escape(str(building.name if building else "ساختمان"))
    total_pages = max(1, (len(items) + PER - 1) // PER)
    ...
    for item in chunk:
        desc = escape(str(item.description or ""))[:50]
        item_name = escape(str(item.name or ""))
        text += f"• <b>{item_name}</b>\n  {desc}\n  قیمت: <b>{escape(str(item.price or 0))}</b> سکه\n\n"
        btn = item.name if len(item.name) <= 28 else item.name[:26] + "…"
...
async def process_buy(callback: CallbackQuery):
    ...
    try:
        ...
        msg = f"❌ خطا در خرید: {escape(type(e).__name__)}: {escape(str(e))}"
