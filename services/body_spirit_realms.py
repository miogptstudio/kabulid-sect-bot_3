...
BODY_REALMS = [
    "جسم خام", "پوست‌آهن", "استخوان‌فولاد", "عضلهٔ حیوان", "خون‌جاری",
    "تاندون‌ابریشم", "مغز استخوان بیدار", "اندام درونی محکم", "بدن اژدها",
    "بدن ققنوس", "بدن تایتان", "بدن نیمه‌خدا", "بدن خدا", "بدن ازلی", "بدن مطلق",
]

SPIRIT_REALMS = [
    "روح خفته", "جرقه آگاهی", "شعله درون", "رود روح", "اقیانوس ذهن",
    "آسمان روحی", "ستاره‌روان", "کهکشان روان", "روح جاودان", "روح ابدی",
    "روح پوچی", "روح خالق", "روح واحد", "روح بی‌نام", "روح مطلق",
]
...
def body_realm_status(tg: int) -> str:
    ...
    lines += ["", "با /bodycult یا «پرورش پوست» XP قلمرو بدن می‌گیری."]
    return chr(10).join(lines)
...
def spirit_realm_status(tg: int) -> str:
    ...
    for i, name in enumerate(SPIRIT_REALMS):
        mark = " ←" if i == ri else ""
        lines.append(f"{i+1}. {name}{mark}")
    lines += ["", "با /trainspirit XP قلمرو روح می‌گیری."]
    return chr(10).join(lines)
